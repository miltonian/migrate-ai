...
exports.UNIT_TEST_BEST_PRACTICES = exports.findTestBlockEndLine = exports.findLineNumber = exports.writeTestsInExistingFile = exports.getCurrentBranch = exports.executeGitCommand = exports.unminimizeCodeFromFile = exports.addOrUpdateAnyMissingTestsAndOverwriteFile = exports.writeTestsToNewFile = exports.findCodeBlock = exports.addCode = exports.minimizeCode = exports.convertCodeStringBackToCode = exports.minimizeCodeByLines = exports.extractCodeAndReferences = exports.parseCodeForASTs = exports.addToGitignore = exports.TOKEN_MAX_LENGTH = exports.delay = void 0;
...
const existingFile_utils_1 = require("./existingFile.utils");
 ...
 */
async function addToGitignore(directoryPath) {
...
async function parseCodeForASTs(filePath, searchValue, typeSearch, searchValueIsAnyIdentifier, debug) {
    ...
    function serializeAST(node, indentLevel = 0) {
        ...
        (function serialize(node, indentLevel = 0) {
            appendOutput(`${" ".repeat(indentLevel * 2)}${ts.SyntaxKind[node.kind]}: ${node
                .getText()
                .trim()}`);
    ...
    function processNode(node) {
        ...
        if (debug) {
            console.log({
                code: text,
                minimizedCode: (0, exports.minimizeCode)(text),
                searchValue,
                // searchRegex,
                filePath,
            });
        ...
        if (!searchValue ||
            ...
            (0, exports.minimizeCode)(text)
                .replace(/[^\w]/g, "")
                .includes((0, exports.minimizeCode)(searchValue).replace(/[^\w]/g, ""))) {
...
exports.parseCodeForASTs = parseCodeForASTs;
const extractCodeAndReferences = async (filePaths, codeIdentifiers, layers = 3, pos) => {
    usedAsts = [];
    allCode = "";
    ...
    for (const filePath of filePaths) {
        for (const codeIdentifier of codeIdentifiers) {
            ...
            codeForContext += await extractCodeFromASTsInPath(filePath, codeIdentifier, layers, 1, pos);
        }
    }
    return codeForContext;
};
exports.extractCodeAndReferences = extractCodeAndReferences;
const extractCodeFromASTsInPath = async (filePath, codeIdentifier, layers, onLayer, pos) => {
    let model = "GPT-4";
    if (layers === onLayer) {
        return "";
    }
    let codeForContext = "";
    const importDeclarations = await parseCodeForASTs(filePath, "", "ImportDeclaration");
    // console.log({importDeclarations, filePath, codeIdentifier})
    const asts = await parseCodeForASTs(filePath, codeIdentifier);
    let astsArray = (0, lodash_1.uniqBy)(asts, "ast").filter(({ ast, astObj }) => {
    ...
    astsArray = astsArray.length
        ? [
            astsArray.find((ast) => ast.astObj["name"] &&
                ast.astObj["name"]["escapedText"] === codeIdentifier) || astsArray[0],
        ]
        : [];
    for (const { ast, path, code } of astsArray.length ? [astsArray[0]] : []) {
        if (usedAsts.includes(ast.slice(0, exports.TOKEN_MAX_LENGTH)) ||
            usedAsts.includes(code.slice(0, exports.TOKEN_MAX_LENGTH))) {
            continue;
        }
        usedAsts.push(ast.slice(0, exports.TOKEN_MAX_LENGTH));
        usedAsts.push(code.slice(0, exports.TOKEN_MAX_LENGTH));
        codeForContext += `/*file:${path}*/`;
        const identifierPattern1 = /Identifier: (\w+)/g;
        let match;
        let identifiers = []; //identifierPattern1.exec(ast) || [];
        ...
        let match2;
        let importIdentifiers = []; //identifierPattern1.exec(ast) || [];
        for (const importDeclaration of importDeclarations) {
            while ((match2 = identifierPattern1.exec(importDeclaration.ast)) !== null) {
                importIdentifiers.push(match2[1].toLowerCase()); // match[1] is the captured group, which is the identifier
            }
        }
        identifiers = (0, lodash_1.uniq)(identifiers).filter((id) => importIdentifiers.includes(id.toLowerCase()));
        for (const identifier of identifiers) {
            const referencedIdentifierASTs = await parseCodeForASTs(path, identifier, "ImportDeclaration:");
            if (!referencedIdentifierASTs.length) {
                continue;
            }
            const findImportPatter = new RegExp(`Identifier:\\s*${identifier}`, "i");
            let imports = referencedIdentifierASTs.filter(({ ast }) => ast.startsWith("ImportDeclaration:") && findImportPatter.test(ast));
            imports = (0, lodash_1.uniqBy)(imports, (i) => "ast");
            imports = imports.filter(({ ast: i, code: i2 }) => !usedAsts.includes(i.slice(0, exports.TOKEN_MAX_LENGTH)) &&
                !usedAsts.includes(i2.slice(0, exports.TOKEN_MAX_LENGTH)));
            usedAsts.push(...imports.map((a) => a.ast.slice(0, exports.TOKEN_MAX_LENGTH)));
            usedAsts.push(...imports.map((a) => a.code.slice(0, exports.TOKEN_MAX_LENGTH)));
            for (const { code } of imports) {
                codeForContext += (0, exports.addCode)(code, model);
                codeForContext += (0, exports.addCode)("\n", model);
            }
        }
        codeForContext += (0, exports.addCode)("\n", model);
        if (onLayer === 1) {
            codeForContext += (0, exports.addCode)(`/*PRIMARY CODE STARTS HERE*/`, model);
        }
        codeForContext += (0, exports.addCode)("\n\n", model);
        codeForContext += (0, exports.addCode)(code, model);
        codeForContext += (0, exports.addCode)("\n\n", model);
        for (const identifier of identifiers) {
            ...
            let imports = referencedIdentifierASTs.filter(({ ast }) => {
                return (ast.startsWith("ImportDeclaration") && findImportPatter.test(ast));
            });
            imports = (0, lodash_1.uniqBy)(imports, (i) => i.ast);
            for (const { ast, code, astObj } of imports) {
                const { identifiers, path: relativePath } = await getIdentifiersAndPathFromImport(path, ast, astObj, code);
                for (const identifier of identifiers) {
                    if (!relativePath) {
                        continue;
                    }
                    ...
                    codeForContext += await extractCodeFromASTsInPath(absolutePath, identifier, layers, onLayer + 1);
                }
            }
        }
    }
    if (onLayer === 1) {
        if (reachedLimit) {
            console.info("Truncating code to copy since it hit the token limit for this model. Try selecting a smaller subset of the code");
        }
    }
    return codeForContext;
};
const getIdentifiersAndPathFromImport = async (fromPath, ast, astObj, code) => {
    ...
    const lastStringLiteralMatch /* path of the import */ = (stringLiteralMatches?.length &&
    ...
    try {
        ...
        const resolvedPath = await resolveImportPath(fromPath, lastStringLiteralMatch || "").catch((err) => {
            console.error(err);
 ...
 * @returns A promise that resolves to the absolute path of the resolved module or undefined if not resolved.
 */
async function resolveImportPath(absoluteFilePath, importStatement) {
    try {
        const text = fs_1.default.readFileSync(absoluteFilePath, "utf-8");
        const sourceFile = ts.createSourceFile(absoluteFilePath, text, ts.ScriptTarget.Latest);
        let resolvedPath;
        const visitNode = (node) => {
            if (ts.isImportDeclaration(node) &&
                ...
                if (!resolvedPath) {
                    resolvedPath = require.resolve(path.join(directoryPath, `${importPath}.ts`));
                }
                if (!resolvedPath) {
                    resolvedPath = require.resolve(path.join(directoryPath, `${importPath}.js`));
                }
            }
...
const minimizeCodeByLines = (code) => {
    return code
        .split("\n")
        .map((line) => line.replace(/"/g, '\\"'))
        .join("\\n");
};
exports.minimizeCodeByLines = minimizeCodeByLines;
const convertCodeStringBackToCode = (str) => {
    return str.replace(/\\n/g, "\n").replace(/\\"/g, '"');
};
exports.convertCodeStringBackToCode = convertCodeStringBackToCode;
const minimizeCode = (code) => {
    // Remove comments
    ...
    code = code.replace(/function\s+(\w+)\s*\(([^)]*)\)\s*\{([^}]*)\}/g, (match, fName, args, body) => `${fName}=${args.trim()}=>${body.trim().replace(/return\s+/, "")}`);
...
exports.minimizeCode = minimizeCode;
const addCode = (newCodeUncompressed, model, isPrimaryCode) => {
    const newCode = (0, exports.minimizeCode)(newCodeUncompressed);
    ...
    const tokenCount = (0, openai_chat_tokens_1.stringTokens)(allCode + newCode);
...
const findCodeBlock = async (filePath, params) => {
    const { startString, lineNumber } = params;
    const parsed = await parseCodeForASTs(filePath, startString, undefined, undefined);
    const result = parsed.find(({ code }) => {
        return (0, exports.minimizeCode)(code)
            .trim()
            .replace(/[^\w]/g, "")
 ...
 */
const writeTestsToNewFile = async (filePath, contextTestFile, autoNewFileName) => {
    ...
    const modifiedTestCodeForContext = await getMinimizedTestCodeWithRefToModifyOrContext(contextTestFile, "new");
    if (!modifiedTestCodeForContext?.codeBlockWithRefToModify) {
        throw new Error("Something went wrong with getting code from test file used for context");
    }
    // add other test file to context
    await (0, openai_utils_1.sendMessageToAssistant)(`also use this test code for context. await my further instructions ${modifiedTestCodeForContext?.codeBlockWithRefToModify || ""}`);
    let codeToAdd = await generateInsertionCode("", // not needed for new test file generation
    "new", newTestFilePath);
    if (codeToAdd) {
        await insertTextToFile(newTestFilePath, 0, codeToAdd); // Insert at the beginning of the new file
        codeToAdd = await (0, exports.addOrUpdateAnyMissingTestsAndOverwriteFile)(newTestFilePath, codeToAdd);
...
exports.writeTestsToNewFile = writeTestsToNewFile;
...
//   return codeToAdd;
// };
const addOrUpdateAnyMissingTestsAndOverwriteFile = async (filePath, code, isPartialCode, overrideOverwrite) => {
    let codeToAdd = code;
    let i = 0;
    let reflectionIterationMax = 0;
    while (i < reflectionIterationMax) {
        const fixedCodeStr = await (0, openai_utils_1.sendMessageToAssistant)(`In the code you added, are there any missing tests? please add them. 
      
      ${isPartialCode ? codeToAdd : ""}
  
        Your response should contain the updated code for ${isPartialCode ? "code pasted above" : "the entire file"} and be in the json format {"code": string}
        `, "gpt-4o", `Your response should only contain code in the json format {"code": string}`);
        let fixedCodeJSON = null;
        try {
            fixedCodeJSON = JSON.parse(fixedCodeStr?.message?.replace(/```json/g, "").replace(/```/g, "") || "");
        }
        catch (error) {
            console.error(error);
        }
        if (fixedCodeJSON?.code) {
            ...
            if (overrideOverwrite) {
                ...
                // await overwriteFileContent(filePath, fixedCode);
                (0, existingFile_utils_1.replaceAllCodeInFile)(filePath, fixedCode);
            }
        }
        i++;
    }
...
exports.addOrUpdateAnyMissingTestsAndOverwriteFile = addOrUpdateAnyMissingTestsAndOverwriteFile;
 ...
 */
async function unminimizeCodeFromFile(filePath) {
    const fileContents = fs_1.default.readFileSync(filePath, "utf8");
    return fileContents;
}
exports.unminimizeCodeFromFile = unminimizeCodeFromFile;
/**
...
filePath, autoNewFileName) {
    ...
    fs_1.default.writeFileSync(newFilePath, ""); // Create an empty file or handle as needed
    return newFilePath;
}
// /**
 ...
 * @param textToInsert The text to be inserted.
 */
async function insertTextToFile(filePath, lineNumber, textToInsert) {
    try {
 ...
 */
async function getLatestCommitSHA(branch, workingDirectory) {
...
const getCurrentBranch = async (workingDirectory) => {
    try {
        const currentBranch = await executeGitCommand("git rev-parse --abbrev-ref HEAD", workingDirectory);
...
const writeTestsInExistingFile = async (testFile
// lineNumber: number
) => {
    // get code for 1. line number of code to modify and 2. generating test code
    const resp = await getMinimizedTestCodeWithRefToModifyOrContext(testFile, "existing");
    console.log({ resp });
    if (!resp) {
        return;
    }
    ...
    //   "existing"
    // );
    const gitDiff2 = index_1.selectedCodeWithoutReferences; /* minimizeCode(
    ...
    index_1.progressBar.update(35);
    const minimizedByLineCodeToAdd = await (0, existingFile_utils_1.generateCodeToInsertIntoExistingFile)(testFile, minimizedCodeBlockWithRefToModify, resp.lineNumber || 0, index_1.fromHighlightedCode ? index_1.selectedCodeWithoutReferences : gitDiff2);
    let codeToAdd = (0, exports.convertCodeStringBackToCode)(minimizedByLineCodeToAdd);
    if (!codeToAdd) {
    ...
    index_1.progressBar.update(10);
    (0, existingFile_utils_1.replaceAllCodeInFile)(testFile, codeToAdd);
    (0, exports.delay)(2000);
    // vscode.commands.executeCommand("editor.action.autoFix");
...
exports.writeTestsInExistingFile = writeTestsInExistingFile;
const getOutlineOfTestFile = async (testFile) => {
    ...
    const minimizedTestFileCode = (0, exports.minimizeCodeByLines)(unminimizedTestFileCode);
    const outlineStr = await (0, openai_utils_1.sendMessageToChatGPT)(`give me an outline of the title of each and every single testing block / individual test in this test code (title being the complete line of code that starts the testing block)
    ...
    try {
        outlineJSON = JSON.parse(outlineStr?.replace("```json", "")?.replace("```", "") || "");
    }
    catch (error) {
        console.error(error);
    }
...
function findLineNumber(filePath, testBlock) {
 ...
 */
function getLineNumberFromPosition(filePath, position) {
...
async function findTestBlockEndLine(filePath, startString) {
    ...
    astsArray = astsArray.length
        ? [
            astsArray.find((ast) => {
                return (0, exports.minimizeCode)(ast.code).startsWith((0, exports.minimizeCode)(startString));
            }) || astsArray[0],
        ]
        : [];
    if (!astsArray.length) {
...
exports.findTestBlockEndLine = findTestBlockEndLine;
const getMinimizedTestCodeWithRefToModifyOrContext = async (testFile, typeOfTestFile) => {
    ...
    const gitDiff = index_1.currentGitDiff;
    const outline = await getOutlineOfTestFile(testFile);
    console.log({ outline, gitDiff });
    let prompt = "";
    switch (typeOfTestFile) {
        case "existing":
            prompt = `i want you to pick the most fitting item in this array ${JSON.stringify(outline)} that relates to the code i've marked as relevant. be as narrow as you can, i would prefer it if you found a specific test block to put this new code in rather than an overarching/umbrella test group. if it makes more sense to create a new test code block/group within the same code i gave you earlier then do that, or if there is code that is very very similar, then add it to that. your response should be the json format {"testBlock": string}. "testBlock" is the "testBlock" string you chose from the array i gave you. your response should be in json format. only json should be in your response. and here are the RELEVANT CHANGES: ${ignoreDiff
                ? (0, exports.minimizeCode)(index_1.selectedCodeWithoutReferences)
                : `
    ...
    try {
        ...
        if (jsonPositionToModify) {
            jsonPositionToModify.lineNumber =
                (await findTestBlockEndLine(testFile, jsonPositionToModify?.testBlock ||
    ...
    const codeBlockToModifyResp = (jsonPositionToModify &&
        (await (0, exports.findCodeBlock)(testFile, {
    ...
    const codeBlockWithRefToModify = codeBlockToModify &&
        (await (0, exports.extractCodeAndReferences)([testFile], [codeBlockToModify], 3));
    return {
...
const getLineNumber = (node, sourceFile) => {
...
async function generateInsertionCode(codeBlockWithRefToModify, // only needed for writing tests to existing file
typeOfTestFile, newTestFileName // only needed if type of test file is new
) {
    const ignoreDiff = index_1.fromHighlightedCode;
    console.log({ currentGitDiff: index_1.currentGitDiff });
    const gitDiff = index_1.currentGitDiff;
    console.log({ fromHighlightedCode: index_1.fromHighlightedCode, selectedCodeWithoutReferences: index_1.selectedCodeWithoutReferences, gitDiff });
    let planningPrompt = "";
    let prompt = "";
    switch (typeOfTestFile) {
        case "existing":
            planningPrompt = `ok based on this, you are a planner that lays out a plan for the software developer to read and write tests from. a very important job because you only want to add USEFUL tests, not just adding tests to add them but actually testing potential issues with the changed code referenced below. please write a item by item plan which specifically makes tests for the code and git diff here: ${ignoreDiff
                ? (0, exports.minimizeCode)(index_1.selectedCodeWithoutReferences || "")
                : `
        actual code: ${index_1.selectedCodeWithoutReferences}
        ` +
        ...
        git diff: ${gitDiff}
        `)}`;
            prompt = `ok based on this and the code below AND the tests you just spoke about adding, please write BRAND NEW TESTS in a BRAND NEW FILE called ${newTestFileName} based on this relevant code ${ignoreDiff
                ? (0, exports.minimizeCode)(index_1.selectedCodeWithoutReferences || "")
                : `code: ${(0, exports.minimizeCode)(index_1.selectedCodeWithoutReferences || "")} with the gitdiff: ${gitDiff}`}. please remember clearly the context of all of the code in this whole conversation but make the tests specifically for this highlighted code: ${(0, exports.minimizeCode)(index_1.selectedCodeWithoutReferences || "")}. fill in the test code completely so i can run tests immediately. Your response should only contain code in this exact JSON FORMAT {"code": string}`;
            break;
        default:
            throw new Error("something went wrong");
            break;
    ...
    console.log(planningPrompt);
    const plan = await (0, openai_utils_1.sendMessageToAssistant)(planningPrompt, "gpt-4o");
    console.log({ planningPrompt, plan, selectedCodeWithoutReferences: index_1.selectedCodeWithoutReferences });
    const codeToAddJSONStr = await (0, openai_utils_1.sendMessageToAssistant)(prompt, "gpt-4o", `Try to use existing routes or other related existing code as much as you can rather than making things up that don't exist. Your response should only contain code in the json format {"code": string}`);
    let codeToAddJSON = null;
    try {
        codeToAddJSON = JSON.parse(codeToAddJSONStr?.message?.replace(/```json/g, "").replace(/```/g, "") ||
            "");
    }
    catch (error) {
        console.error(error);
 ...
 * @param textToInsert The text to be inserted.
 */
async function insertTextAfterLine(filePath, lineNumber, textToInsert) {
    const fileStream = fs_1.default.createReadStream(filePath);
    const rl = readline_1.default.createInterface({
        input: fileStream,
        crlfDelay: Infinity,
...
exports.UNIT_TEST_BEST_PRACTICES = `
